...
func TestReleaseWorkflowWiresHomebrewTapPublishing(t *testing.T) {
	...
	support.RequireContains(t, workflow, `dist/release-source/scripts/build-release --version "${{ steps.release-version.outputs.version }}" --output-dir dist/release`)
	support.RequireContains(t, workflow, `gh release upload "${version}" dist/release-source/dist/release/* --clobber`)
