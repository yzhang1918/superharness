...
stable schema files plus a central index, and use the same generated contract
surface to update or generate reference docs. The first slice should improve
schema quality and documentation discoverability without changing the current
behavioral contract or tightening existing plain-string fields into enums.
...
- Add generation and validation tooling so schema artifacts and generated docs
  can be regenerated deterministically and checked for drift in tests or CI.
- Update the repository docs so contract reference material comes from the
  generated schema surface instead of hand-maintained duplicated field tables.
...
Use the generated schema surface to produce or drive the contract reference
docs so the repository stops maintaining duplicate field definitions by hand.
...
meaning, ownership, and non-goals, but move repetitive schema-shaped reference
material to generated outputs or generated includes. The resulting docs should
help a cold reader find the schema index, understand the contract families that
are covered, and distinguish public CLI/local-JSON contract surfaces from the
...
- A cold reader can navigate from the main docs to the generated schema index
  and reference docs without needing discovery chat.
- Covered reference docs are generated from or directly driven by the schema
  registry rather than manually duplicated.
- Drift checks fail when generated docs are stale relative to the current
  schema registry.

#### Execution Notes

The same sync path now also generates field-level reference docs under
`docs/reference/contracts/`, including a generated index that links the schema
registry, per-schema docs, and the Go contract module. Repository docs in
`README.md`, `docs/specs/index.md`, `docs/specs/cli-contract.md`, and
`docs/specs/state-model.md` now point readers at the generated contract
reference instead of implying that prose specs remain the field-level source of
truth for covered JSON contracts. Validation:
`scripts/sync-contract-artifacts` and
`scripts/sync-contract-artifacts --check`.
...
- Add deterministic generation commands plus drift checks for both schema and
  generated docs, and run the relevant targeted test suites that cover the
  touched command packages.
...
- Risk: Docs may remain partially duplicated if generated references and prose
  ...
  - Mitigation: Update the docs ownership model during execution so prose keeps
    normative meaning while generated references own field-by-field contract
